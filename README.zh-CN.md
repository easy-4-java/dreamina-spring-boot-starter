...
**Spring Boot Starter，集成 dreamina**
...
**dreamina-spring-boot-starter** 是一个面向 使用 dreamina 的应用 的 Spring Boot Starter，用于将 **dreamina** 集成到 Spring Boot 应用中。它提供自动装配、属性绑定与开箱即用的 Bean，使应用以最小配置即可使用 dreamina 的全部能力。
...
| JDK | `17` |
| 坐标 | `io.github.easy4j:dreamina-spring-boot-starter:3.3.x.20260630-SNAPSHOT` |
| 配置前缀 | `dreamina` |
...
| 能力 | 状态 | 说明 |
|---|:---:|---|
| 自动装配 | ✅ 稳定 | 自动注册 dreamina 相关 Bean |
| 属性绑定 | ✅ 稳定 | 绑定 `dreamina.*` 到 `DreaminaProperties` |
| `DreaminaCliExecutor` Bean | ✅ 稳定 | 通过 DreaminaAutoConfiguration 自动注册 |
...
| Bean | 条件 | 缺失时行为 |
|---|---|---|
| `DreaminaCliExecutor` | classpath + property | 不创建 |
| `DreaminaCliAvailabilityChecker` | classpath + property | 不创建 |
| `DreaminaCliStartupChecker` | classpath + property | 不创建 |
...
| 属性 | 类型 | 默认值 | 必填 | 说明 | 敏感 |
|---|---|---|:---:|---|:---:|
| `dreamina.enabled` | boolean | `true` | 否 | 是否启用 Starter | 否 |
<!-- 更多属性见下方 -->
...
| 症状 | 诊断 | 解决 |
|---|---|---|
| Bean 未创建 | 查看自动装配报告 | 确认 `dreamina.enabled=true` 与 classpath |
